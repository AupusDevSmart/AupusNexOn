...
// ================================================================
// FORK TON-V2 do iot-firmware-generator.v2.js — gerador da placa SCH-TON-v1b.
// So enxerga os tipos ton1v2..ton4v2 (o V1 segue dono dos ton1..ton4).
// TON_CAPS/tonCaps: canonicos em iot-diagram.v2.js (carrega por ultimo e define
// o global com v1+v2 e contagens) — este fork NAO os redeclara de proposito.
...
var FirmwareGeneratorTonV2 = class FirmwareGeneratorTonV2 {
    ...
    analyze() {
        ...
        const tonTypes = ['ton1v2', 'ton2v2', 'ton3v2', 'ton4v2'];
    ...
    _processNetwork(ton, other, result, components, connections) {
        if (other.type === 'wifi_router') {
            ...
            result.wifi = {
                ssid: nets[0] ? nets[0].ssid : (other.props.ssid || ''),
    ...
    generateProject(spec) {
        ...
        const files = {};
        for (const [path, content] of Object.entries(FIRMWARE_BASE_TON_V2)) {
            files[path] = content;
        }
...
#define FIRMWARE_VERSION    "${(() => { const d = new Date(); const p = n => String(n).padStart(2,'0'); return `2.0.0-build${d.getFullYear()}${p(d.getMonth()+1)}${p(d.getDate())}${p(d.getHours())}${p(d.getMinutes())}`; })()}"
...
// PWM (V2: 8 canais via PCA9685@0x42 — ver pwm.h no base)
#define PWM_COUNT_V2        8

// ADC
#define ADC_RESOLUTION_BITS 12
#define ADC_ATTEN           ADC_11db
#define ADC_DIVIDER         7.67

// MCP23008 pin mapping
#define MCP_INPUT_COUNT     8
...
`;
    ...
    // V2: "pwm<N> <0-100>" — duty em % no canal N (1..8) do PCA9685 (X88).
    // "pwm<N> off" = 0%. Ex.: "pwm3 75". (Contrato: decisao 2026-07-21, 0-100%.)
    if (cmd.startsWith("pwm") && cmd.length() >= 4 && cmd[3] >= '1' && cmd[3] <= '8') {
        uint8_t ch = cmd[3] - '0';
        int pct = -1;
        if (cmd.indexOf("off") >= 0) pct = 0;
        else {
            int sp = cmd.indexOf(' ');
            if (sp > 0) pct = cmd.substring(sp + 1).toInt();
        }
        if (pct < 0 || pct > 100) { snprintf(result_msg, msg_sz, "pwm_pct_invalido"); return false; }
        if (!pwm_set_percent(ch, (uint8_t)pct)) { snprintf(result_msg, msg_sz, "pwm_fail"); return false; }
        snprintf(result_msg, msg_sz, "pwm%c_%d", cmd[3], pct);
        return true;
    }
`;
...
`;
        ...
        // ===== Comando do carregador: {carregador:habilitar|desabilitar} =====
        if (spec.carregador && spec.has_relays) {
...
void setup() {
    ...
    // Perifericos
    if (inputs_init()) Serial.println("[OK] Entradas (8x optoacopladas + SU+)");
...
`;
    ...
    adc_init();
    Serial.println("[OK] ADC (AN1/AN2 + AN_C1/AN_C2)");

    if (pwm_init()) Serial.println("[OK] PWM (PCA9685 8ch @0x42)");
    else Serial.println("[FAIL] PWM (PCA9685 nao respondeu no 0x42)");
...
`;
        ...
        bool _su_ch = su_changed();            // idem (consome o flag)
        if (_io_force || _in_changed || _su_ch) {
            uint8_t s = inputs_get_state();
            char buf[144];
            snprintf(buf, sizeof(buf), "{\\"d1\\":%d,\\"d2\\":%d,\\"d3\\":%d,\\"d4\\":%d,\\"d5\\":%d,\\"d6\\":%d,\\"d7\\":%d,\\"d8\\":%d,\\"s1\\":%d}",
                s&1, (s>>1)&1, (s>>2)&1, (s>>3)&1, (s>>4)&1, (s>>5)&1, (s>>6)&1, (s>>7)&1, su_get_state() ? 1 : 0);
`;
...
`;
        if (spec.has_relays) {
            cpp += `
        // Reles (R1-8) on-change. V2: bit (n-1) = RLn.
        uint8_t rl = relays_get_state();
        if (_io_force || rl != _prev_rl) {
            _prev_rl = rl;
            char rbuf[128];
            snprintf(rbuf, sizeof(rbuf), "{\\"r1\\":%d,\\"r2\\":%d,\\"r3\\":%d,\\"r4\\":%d,\\"r5\\":%d,\\"r6\\":%d,\\"r7\\":%d,\\"r8\\":%d}",
                rl&1, (rl>>1)&1, (rl>>2)&1, (rl>>3)&1, (rl>>4)&1, (rl>>5)&1, (rl>>6)&1, (rl>>7)&1);
`;
