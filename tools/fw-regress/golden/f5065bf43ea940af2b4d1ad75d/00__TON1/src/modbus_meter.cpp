...
// =========================================================================
// Inversor (WEG SIW500H ST060)
// Endereco Modbus: 1
// Modos: 25 avg, 12 last, 0 delta
// =========================================================================
...
static bool _read_dev_0_raw(uint16_t *buf) {
    _select(1);
    // Bloco 0: reg 32016, count 24, func 0x03 - Regs 32016-32039: PV1-12 V/I (6 MPPTs x 2 strings)
    {
        uint8_t rc = _mb.readHoldingRegisters(32016, 24);
        if (rc != _mb.ku8MBSuccess) {
            ...
            if (_ds0.fail_streak == 0) {
                rc = _mb.readHoldingRegisters(32016, 24);
            }
        }
        if (rc == _mb.ku8MBSuccess) {
            for (uint16_t i = 0; i < 24; i++) buf[0 + i] = _mb.getResponseBuffer(i);
            diag_modbus_ok++;
        } else {
            diag_modbus_err++;
            Serial.printf("[MB] Inversor_1 bloco 0 FAIL (reg=32016 count=24 rc=0x%02X)\n", rc);
            return false;
        }
    }
    ...
    // Bloco 1: reg 32064, count 27, func 0x03 - Regs 32064-32090: P, V, I, freq, temp, isolamento, status, fault
    {
        ...
        if (rc == _mb.ku8MBSuccess) {
            for (uint16_t i = 0; i < 27; i++) buf[24 + i] = _mb.getResponseBuffer(i);
            diag_modbus_ok++;
        } else {
            diag_modbus_err++;
            Serial.printf("[MB] Inversor_1 bloco 1 FAIL (reg=32064 count=27 rc=0x%02X)\n", rc);
            return false;
        }
    }
    // Espacamento entre blocos. 80ms cobre datalogger interno do Sungrow CX
    ...
    // Bloco 2: reg 32106, count 10, func 0x03 - Regs 32106-32115: energia total e diaria
    {
        ...
        if (rc == _mb.ku8MBSuccess) {
            for (uint16_t i = 0; i < 10; i++) buf[51 + i] = _mb.getResponseBuffer(i);
            diag_modbus_ok++;
        } else {
            diag_modbus_err++;
            Serial.printf("[MB] Inversor_1 bloco 2 FAIL (reg=32106 count=10 rc=0x%02X)\n", rc);
            return false;
        }
    }
    // Espacamento entre blocos. 80ms cobre datalogger interno do Sungrow CX
    ...
    // Bloco 3: reg 30070, count 5, func 0x03 - Regs 30070-30074: Model ID + Rated Power
    {
        ...
        if (rc == _mb.ku8MBSuccess) {
            for (uint16_t i = 0; i < 5; i++) buf[61 + i] = _mb.getResponseBuffer(i);
            diag_modbus_ok++;
        } else {
            diag_modbus_err++;
            Serial.printf("[MB] Inversor_1 bloco 3 FAIL (reg=30070 count=5 rc=0x%02X)\n", rc);
            return false;
        }
    }
    // Espacamento entre blocos. 80ms cobre datalogger interno do Sungrow CX
...
static void _sample_dev_0() {
    ...
    float v_ia = (float)(int32_t)(((uint32_t)buf[32] << 16) | buf[32+1]) / 1000.0;
    _ds0.sum_[0] += v_ia;
    float v_ib = (float)(int32_t)(((uint32_t)buf[34] << 16) | buf[34+1]) / 1000.0;
    _ds0.sum_[1] += v_ib;
    float v_ic = (float)(int32_t)(((uint32_t)buf[36] << 16) | buf[36+1]) / 1000.0;
    _ds0.sum_[2] += v_ic;
    float v_vab = (float)buf[26] / 10.0;
    _ds0.sum_[3] += v_vab;
    float v_vbc = (float)buf[27] / 10.0;
    _ds0.sum_[4] += v_vbc;
    float v_vca = (float)buf[28] / 10.0;
    _ds0.sum_[5] += v_vca;
    ...
    float v_dc_total_power = (float)(int32_t)(((uint32_t)buf[24] << 16) | buf[24+1]);
    _ds0.sum_[12] += v_dc_total_power;
    float v_string1_current = (float)(int16_t)buf[1] / 100.0;
    _ds0.sum_[13] += v_string1_current;
    float v_string2_current = (float)(int16_t)buf[3] / 100.0;
    _ds0.sum_[14] += v_string2_current;
    float v_string3_current = (float)(int16_t)buf[5] / 100.0;
    _ds0.sum_[15] += v_string3_current;
    float v_string4_current = (float)(int16_t)buf[7] / 100.0;
    _ds0.sum_[16] += v_string4_current;
    float v_string5_current = (float)(int16_t)buf[9] / 100.0;
    _ds0.sum_[17] += v_string5_current;
    float v_string6_current = (float)(int16_t)buf[11] / 100.0;
    _ds0.sum_[18] += v_string6_current;
    float v_string7_current = (float)(int16_t)buf[13] / 100.0;
    _ds0.sum_[19] += v_string7_current;
    float v_string8_current = (float)(int16_t)buf[15] / 100.0;
    _ds0.sum_[20] += v_string8_current;
    float v_string9_current = (float)(int16_t)buf[17] / 100.0;
    _ds0.sum_[21] += v_string9_current;
    float v_string10_current = (float)(int16_t)buf[19] / 100.0;
    _ds0.sum_[22] += v_string10_current;
    float v_string11_current = (float)(int16_t)buf[21] / 100.0;
    _ds0.sum_[23] += v_string11_current;
    float v_string12_current = (float)(int16_t)buf[23] / 100.0;
    _ds0.sum_[24] += v_string12_current;
    float v_fp = (float)(int16_t)buf[44] / 1000.0;
    float v_freq = (float)buf[45] / 100.0;
    float v_work_state = (float)buf[49];
    float v_daily_yield = (float)(((uint32_t)buf[59] << 16) | buf[59+1]) / 100.0;
    float v_device_type = (float)buf[61];
    float v_total_yield = (float)(((uint32_t)buf[51] << 16) | buf[51+1]) / 100.0;
    float v_temp_interna = (float)(int16_t)buf[47] / 10.0;
    float v_nominal_power = (float)(((uint32_t)buf[64] << 16) | buf[64+1]) / 1000.0;
    float v_pid_alarm_code = (float)buf[50];
    float v_potencia_ativa = (float)(int32_t)(((uint32_t)buf[40] << 16) | buf[40+1]);
    float v_potencia_reativa = (float)(int32_t)(((uint32_t)buf[42] << 16) | buf[42+1]);
    float v_insulation_resistance = (float)buf[48] / 1000.0;
    _ds0.last_[0] = v_ia;
    _ds0.last_[1] = v_ib;
    _ds0.last_[2] = v_ic;
    _ds0.last_[3] = v_vab;
    ...
    _ds0.last_[12] = v_dc_total_power;
    _ds0.last_[13] = v_string1_current;
    _ds0.last_[14] = v_string2_current;
    _ds0.last_[15] = v_string3_current;
    _ds0.last_[16] = v_string4_current;
    _ds0.last_[17] = v_string5_current;
    _ds0.last_[18] = v_string6_current;
    _ds0.last_[19] = v_string7_current;
    _ds0.last_[20] = v_string8_current;
    _ds0.last_[21] = v_string9_current;
    _ds0.last_[22] = v_string10_current;
    _ds0.last_[23] = v_string11_current;
    _ds0.last_[24] = v_string12_current;
    _ds0.last_[25] = v_fp;
    _ds0.last_[26] = v_freq;
    _ds0.last_[27] = v_work_state;
    _ds0.last_[28] = v_daily_yield;
    _ds0.last_[29] = v_device_type;
    _ds0.last_[30] = v_total_yield;
    _ds0.last_[31] = v_temp_interna;
    _ds0.last_[32] = v_nominal_power;
    _ds0.last_[33] = v_pid_alarm_code;
    _ds0.last_[34] = v_potencia_ativa;
    _ds0.last_[35] = v_potencia_reativa;
    _ds0.last_[36] = v_insulation_resistance;
    _ds0.samples++;
...
static void _publish_dev_0(modbus_publish_fn publish) {
    ...
    { char _h[8]; snprintf(_h, sizeof(_h), "%x", (uint16_t)(_ds0.last_[29])); g_info["device_type"] = String(_h); }
    g_info["nominal_power"] = _ds0.last_[32];
    JsonObject g_energy = d.createNestedObject("energy");
    g_energy["daily_yield"] = _ds0.last_[28];
    ...
    g_dc["total_power"] = (float)(_ds0.sum_[12] / n);
    g_dc["string1_current"] = (float)(_ds0.sum_[13] / n);
    g_dc["string2_current"] = (float)(_ds0.sum_[14] / n);
    g_dc["string3_current"] = (float)(_ds0.sum_[15] / n);
    g_dc["string4_current"] = (float)(_ds0.sum_[16] / n);
    g_dc["string5_current"] = (float)(_ds0.sum_[17] / n);
    g_dc["string6_current"] = (float)(_ds0.sum_[18] / n);
    g_dc["string7_current"] = (float)(_ds0.sum_[19] / n);
    g_dc["string8_current"] = (float)(_ds0.sum_[20] / n);
    g_dc["string9_current"] = (float)(_ds0.sum_[21] / n);
    g_dc["string10_current"] = (float)(_ds0.sum_[22] / n);
    g_dc["string11_current"] = (float)(_ds0.sum_[23] / n);
    g_dc["string12_current"] = (float)(_ds0.sum_[24] / n);
    ...
    g_power["power_factor"] = _ds0.last_[25];
    g_power["frequency"] = _ds0.last_[26];
    g_power["active_total"] = _ds0.last_[34];
    g_power["reactive_total"] = _ds0.last_[35];
    JsonObject g_status = d.createNestedObject("status");
    g_status["work_state"] = _ds0.last_[27];
    g_status["work_state_text"] = _work_state_text((uint16_t)_ds0.last_[27]);
    JsonObject g_protection = d.createNestedObject("protection");
    g_protection["insulation_resistance"] = _ds0.last_[36];
    JsonObject g_pid = d.createNestedObject("pid");
    g_pid["alarm_code"] = _ds0.last_[33];
    ...
    for (int i = 0; i < 25; i++) _ds0.sum_[i] = 0;
    _ds0.samples = 0;
}

// =========================================================================
// Inversor (Huawei SUN2000-100KTL M1)
// Endereco Modbus: 2
