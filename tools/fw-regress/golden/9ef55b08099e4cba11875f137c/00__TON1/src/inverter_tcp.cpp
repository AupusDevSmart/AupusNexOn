...
#define GATEWAY_IP      "192.168.0.114"
...
// =============================================================================
// Inversor — Sungrow SG110CX (slave 1)
...
static bool _read_tcp_inv_0_raw(uint16_t *buf) {
    ...
    {
        uint16_t tmp0[50];
        if (!_modbus_tcp_read("192.168.0.114", 502, 2000, 1, 0x04, 4999, 50, tmp0)) {
    ...
    {
        uint16_t tmp1[40];
        if (!_modbus_tcp_read("192.168.0.114", 502, 2000, 1, 0x04, 5049, 40, tmp1)) {
    ...
    {
        uint16_t tmp2[30];
        if (!_modbus_tcp_read("192.168.0.114", 502, 2000, 1, 0x04, 5089, 30, tmp2)) {
    ...
    {
        uint16_t tmp3[35];
        if (!_modbus_tcp_read("192.168.0.114", 502, 2000, 1, 0x04, 5119, 35, tmp3)) {
    ...
    {
        uint16_t tmp4[18];
        if (!_modbus_tcp_read("192.168.0.114", 502, 2000, 1, 0x04, 7012, 18, tmp4)) {
