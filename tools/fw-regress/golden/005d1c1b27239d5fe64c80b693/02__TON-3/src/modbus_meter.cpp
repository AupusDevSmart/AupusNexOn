...
static bool _read_dev_0_raw(uint16_t *buf) {
    ...
    // Bloco 0: reg 15, count 6, func 0x04 - RTU 30016-30021: Va/Vb/Vc Primary
    {
        ...
        if (rc == _mb.ku8MBSuccess) {
            ...
            diag_modbus_err++;
            Serial.printf("[MB] Rele DJ-3.1_1 bloco 0 FAIL (reg=15 count=6 rc=0x%02X)\n", rc);
            return false;
        }
    }
    ...
    // Bloco 1: reg 59, count 2, func 0x04 - RTU 30060: Frequency Hz
    {
        ...
        if (rc == _mb.ku8MBSuccess) {
            ...
            diag_modbus_err++;
            Serial.printf("[MB] Rele DJ-3.1_1 bloco 1 FAIL (reg=59 count=2 rc=0x%02X)\n", rc);
            return false;
        }
    }
    // Espacamento entre blocos. 80ms cobre datalogger interno do Sungrow CX
    ...
    // Bloco 2: reg 63, count 6, func 0x04 - RTU 30064-30069: Ia/Ib/Ic Primary
    {
        ...
        if (rc == _mb.ku8MBSuccess) {
            ...
            diag_modbus_err++;
            Serial.printf("[MB] Rele DJ-3.1_1 bloco 2 FAIL (reg=63 count=6 rc=0x%02X)\n", rc);
            return false;
        }
    }
    // Espacamento entre blocos. 80ms cobre datalogger interno do Sungrow CX
    ...
    // Bloco 3: reg 87, count 2, func 0x04 - RTU 30088: In Primary
    {
        ...
        if (rc == _mb.ku8MBSuccess) {
            ...
            diag_modbus_err++;
            Serial.printf("[MB] Rele DJ-3.1_1 bloco 3 FAIL (reg=87 count=2 rc=0x%02X)\n", rc);
            return false;
        }
    }
    // Espacamento entre blocos. 80ms cobre datalogger interno do Sungrow CX
    ...
    // Bloco 4: reg 117, count 2, func 0x04 - RTU 30118: P 3Ph Primary
    {
        ...
        if (rc == _mb.ku8MBSuccess) {
            ...
            diag_modbus_err++;
            Serial.printf("[MB] Rele DJ-3.1_1 bloco 4 FAIL (reg=117 count=2 rc=0x%02X)\n", rc);
            return false;
        }
    }
    // Espacamento entre blocos. 80ms cobre datalogger interno do Sungrow CX
    ...
    // Bloco 5: reg 125, count 2, func 0x04 - RTU 30126: Q 3Ph Primary
    {
        ...
        if (rc == _mb.ku8MBSuccess) {
            ...
            diag_modbus_err++;
            Serial.printf("[MB] Rele DJ-3.1_1 bloco 5 FAIL (reg=125 count=2 rc=0x%02X)\n", rc);
            return false;
        }
    }
    // Espacamento entre blocos. 80ms cobre datalogger interno do Sungrow CX
    ...
    // Bloco 6: reg 135, count 6, func 0x04 - RTU 30136-30141: PF PhA/B/C
    {
        ...
        if (rc == _mb.ku8MBSuccess) {
            ...
            diag_modbus_err++;
            Serial.printf("[MB] Rele DJ-3.1_1 bloco 6 FAIL (reg=135 count=6 rc=0x%02X)\n", rc);
            return false;
        }
    }
    // Espacamento entre blocos. 80ms cobre datalogger interno do Sungrow CX
...
static void _sample_dev_0() {
    ...
    if (!_read_dev_0_raw(buf)) {
        _ds0.fail_streak++;
        Serial.printf("[MB] Rele DJ-3.1_1: falha leitura (consecutivas: %d)\n", _ds0.fail_streak);
        if (_ds0.fail_streak >= MODBUS_FAIL_COOLDOWN_N) {
            ...
            Serial.printf("[MB] Rele DJ-3.1_1: cooldown %lus (nao responde) — loop liberado p/ LoRa/cmd\n",
    ...
    if (_ds0.fail_streak > 0) {
        Serial.printf("[MB] Rele DJ-3.1_1: OK (apos %d falhas consecutivas)\n", _ds0.fail_streak);
        _ds0.fail_streak = 0;
    }
    ...
    Serial.printf("[MB] Rele DJ-3.1_1 #%d: ", _ds0.samples);
...
static void _publish_dev_0(modbus_publish_fn publish) {
    if (!_ds0.valid || _ds0.samples == 0) {
        publish("Rele DJ-3.1_1/status", "{\"error\":\"no_samples\"}");
        return;
    }
    ...
    if (sz > 0) {
        publish("Rele DJ-3.1_1/data", payload);
        Serial.printf("\n===== PUBLICADO: Rele DJ-3.1_1 (%d amostras) =====\n", n);
...
static void _evt_emit_dev_0(modbus_publish_fn publish, const uint8_t* b) {
    ...
    n += snprintf(p + n, sizeof(p) - n,
        "{\"dev\":\"Rele DJ-3.1_1\",\"cat\":\"siemens-7sr5\",\"proto\":\"modbus_7sr\""
        ",\"type\":%u,\"fun\":%u,\"inf\":%u"
    ...
    Serial.printf("[EVT] Rele DJ-3.1_1 t%u FUN=%u INF=%u %02u:%02u:%06.3f%s\n",
...
static void _evt_poll_dev_0(modbus_publish_fn publish) {
    ...
    if (cnt == 0) return;
    Serial.printf("[EVT] Rele DJ-3.1_1: %u evento(s) no buffer\n", (unsigned)cnt);
    for (int guard = 0; guard < 32; guard++) {   // teto: nunca travar o loop principal
        ...
        if (rc != _mb.ku8MBSuccess) {
            Serial.printf("[EVT] Rele DJ-3.1_1: leitura de evento rc=0x%02X — aborta ciclo\n", rc);
            break;
        }
        uint8_t b[16];
        for (int i = 0; i < 8; i++) {
...
bool modbus_exec_command(const char* device_name, const char* cmd_id) {
    ...
    if (strcmp(device_name, "Rele DJ-3.1") == 0) {
        _select(1);
        return false;
    }
    return false;
