...
#define GATEWAY_IP      "192.168.0.104"
...
// =============================================================================
// Inversor — Sungrow SG250CX (slave 1)
...
static bool _read_tcp_inv_0_raw(uint16_t *buf) {
    ...
    {
        uint16_t tmp0[50];
        if (!_modbus_tcp_read("192.168.0.104", 502, 2000, 1, 0x04, 4999, 50, tmp0)) {
            Serial.printf("[TCP-INV] Inversor(id1) bloco 0 FAIL (reg=4999 count=50)\n");
            return false;
        }
    ...
    {
        uint16_t tmp1[40];
        if (!_modbus_tcp_read("192.168.0.104", 502, 2000, 1, 0x04, 5049, 40, tmp1)) {
            Serial.printf("[TCP-INV] Inversor(id1) bloco 1 FAIL (reg=5049 count=40)\n");
            return false;
        }
    ...
    {
        uint16_t tmp2[30];
        if (!_modbus_tcp_read("192.168.0.104", 502, 2000, 1, 0x04, 5089, 30, tmp2)) {
            Serial.printf("[TCP-INV] Inversor(id1) bloco 2 FAIL (reg=5089 count=30)\n");
            return false;
        }
    ...
    {
        uint16_t tmp3[35];
        if (!_modbus_tcp_read("192.168.0.104", 502, 2000, 1, 0x04, 5119, 35, tmp3)) {
            Serial.printf("[TCP-INV] Inversor(id1) bloco 3 FAIL (reg=5119 count=35)\n");
            return false;
        }
    ...
    {
        uint16_t tmp4[18];
        if (!_modbus_tcp_read("192.168.0.104", 502, 2000, 1, 0x04, 7012, 18, tmp4)) {
            Serial.printf("[TCP-INV] Inversor(id1) bloco 4 FAIL (reg=7012 count=18)\n");
            return false;
        }
...
static void _sample_tcp_inv_0() {
    ...
    if (!_read_tcp_inv_0_raw(buf)) {
        ...
        _tds0.fail_streak++;
        Serial.printf("[TCP-INV] Inversor(id1): falha leitura (consecutivas: %d)\n", _tds0.fail_streak);
        if (_tds0.fail_streak >= MODBUS_FAIL_COOLDOWN_N) {
            ...
            Serial.printf("[TCP-INV] Inversor(id1): cooldown %lus (nao responde) — loop liberado p/ MQTT/cmd\n", (unsigned long)(MODBUS_COOLDOWN_MS/1000));
        }
        return;
    }
    _tds0.cooldown_until = 0;
    if (_tds0.fail_streak > 0) {
        Serial.printf("[TCP-INV] Inversor(id1): OK (apos %d falhas)\n", _tds0.fail_streak);
        _tds0.fail_streak = 0;
    }
    ...
    _tds0.samples++;
    _tds0.valid = true;
    Serial.printf("[TCP-INV] Inversor(id1) #%d: ", _tds0.samples);
...
static void _publish_tcp_inv_0(tcp_publish_fn publish) {
    if (!_tds0.valid || _tds0.samples == 0) {
        ...
        publish("Inversor_1/data", _nsbuf);
        return;
    }
...
// =============================================================================
// Inversor — Sungrow SG250CX (slave 2)
// 5 blocos | 55 pontos AI (34 avg, 21 last, 0 delta)
// word_order: low_first
// =============================================================================
struct _TcpDs1State {
...
static bool _read_tcp_inv_1_raw(uint16_t *buf) {
    ...
    {
        uint16_t tmp0[50];
        if (!_modbus_tcp_read("192.168.0.104", 502, 2000, 2, 0x04, 4999, 50, tmp0)) {
            Serial.printf("[TCP-INV] Inversor(id2) bloco 0 FAIL (reg=4999 count=50)\n");
            return false;
        }
        for (uint16_t i = 0; i < 50; i++) buf[0 + i] = tmp0[i];
    }
    ...
    {
        uint16_t tmp1[40];
        if (!_modbus_tcp_read("192.168.0.104", 502, 2000, 2, 0x04, 5049, 40, tmp1)) {
            Serial.printf("[TCP-INV] Inversor(id2) bloco 1 FAIL (reg=5049 count=40)\n");
            return false;
        }
        for (uint16_t i = 0; i < 40; i++) buf[50 + i] = tmp1[i];
    }
    ...
    {
        uint16_t tmp2[30];
        if (!_modbus_tcp_read("192.168.0.104", 502, 2000, 2, 0x04, 5089, 30, tmp2)) {
            Serial.printf("[TCP-INV] Inversor(id2) bloco 2 FAIL (reg=5089 count=30)\n");
            return false;
        }
        for (uint16_t i = 0; i < 30; i++) buf[90 + i] = tmp2[i];
    }
    ...
    {
        uint16_t tmp3[35];
        if (!_modbus_tcp_read("192.168.0.104", 502, 2000, 2, 0x04, 5119, 35, tmp3)) {
            Serial.printf("[TCP-INV] Inversor(id2) bloco 3 FAIL (reg=5119 count=35)\n");
            return false;
        }
        for (uint16_t i = 0; i < 35; i++) buf[120 + i] = tmp3[i];
    }
    ...
    {
        uint16_t tmp4[18];
        if (!_modbus_tcp_read("192.168.0.104", 502, 2000, 2, 0x04, 7012, 18, tmp4)) {
            Serial.printf("[TCP-INV] Inversor(id2) bloco 4 FAIL (reg=7012 count=18)\n");
            return false;
        }
        for (uint16_t i = 0; i < 18; i++) buf[155 + i] = tmp4[i];
    }
...
static void _sample_tcp_inv_1() {
    ...
    if (!_read_tcp_inv_1_raw(buf)) {
        ...
        _tds1.fail_streak++;
        Serial.printf("[TCP-INV] Inversor(id2): falha leitura (consecutivas: %d)\n", _tds1.fail_streak);
        if (_tds1.fail_streak >= MODBUS_FAIL_COOLDOWN_N) {
            ...
            Serial.printf("[TCP-INV] Inversor(id2): cooldown %lus (nao responde) — loop liberado p/ MQTT/cmd\n", (unsigned long)(MODBUS_COOLDOWN_MS/1000));
        }
        return;
    }
    _tds1.cooldown_until = 0;
    if (_tds1.fail_streak > 0) {
        Serial.printf("[TCP-INV] Inversor(id2): OK (apos %d falhas)\n", _tds1.fail_streak);
        _tds1.fail_streak = 0;
    }
    ...
    _tds1.samples++;
    _tds1.valid = true;
    Serial.printf("[TCP-INV] Inversor(id2) #%d: ", _tds1.samples);
    Serial.printf("vab=%.2f ", v_vab);
    Serial.printf("vbc=%.2f ", v_vbc);
    Serial.printf("vca=%.2f ", v_vca);
    Serial.printf("ia=%.2f ", v_ia);
...
static void _publish_tcp_inv_1(tcp_publish_fn publish) {
    if (!_tds1.valid || _tds1.samples == 0) {
        ...
        publish("Inversor_2/data", _nsbuf);
        return;
    }
