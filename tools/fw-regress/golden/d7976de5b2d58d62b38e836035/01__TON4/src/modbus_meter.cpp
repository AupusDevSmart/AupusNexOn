...
static bool _read_dev_1_raw(uint16_t *buf) {
    ...
    // Bloco 0: reg 37, count 34, func 0x03 - V, I, P, Q, FP, S, Energia (37..70)
    {
        ...
        if (rc == _mb.ku8MBSuccess) {
            ...
            diag_modbus_err++;
            Serial.printf("[MB] Power Meter_2 bloco 0 FAIL (reg=37 count=34 rc=0x%02X)\n", rc);
            return false;
        }
    }
    // Espacamento entre blocos. 80ms cobre datalogger interno do Sungrow CX
...
static void _sample_dev_1() {
    ...
    if (!_read_dev_1_raw(buf)) {
        _ds1.fail_streak++;
        Serial.printf("[MB] Power Meter_2: falha leitura (consecutivas: %d)\n", _ds1.fail_streak);
        if (_ds1.fail_streak >= MODBUS_FAIL_COOLDOWN_N) {
            ...
            Serial.printf("[MB] Power Meter_2: cooldown %lus (nao responde) — loop liberado p/ LoRa/cmd\n",
                          (unsigned long)(MODBUS_COOLDOWN_MS/1000));
        }
        return;
    }
    ...
    if (_ds1.fail_streak > 0) {
        Serial.printf("[MB] Power Meter_2: OK (apos %d falhas consecutivas)\n", _ds1.fail_streak);
        _ds1.fail_streak = 0;
    }
    ...
    _ds1.samples++;
    // Log de debug — mostra principais valores lidos
    Serial.printf("[MB] Power Meter_2 #%d: ", _ds1.samples);
    Serial.printf("va=%.2f ", v_va);
    Serial.printf("vb=%.2f ", v_vb);
    Serial.printf("vc=%.2f ", v_vc);
    Serial.printf("ia=%.2f ", v_ia);
...
static void _publish_dev_1(modbus_publish_fn publish) {
    if (!_ds1.valid || _ds1.samples == 0) {
        publish("Power Meter_2/status", "{\"error\":\"no_samples\"}");
        return;
    }
    ...
    size_t sz = serializeJson(d, payload, 3072);
    if (sz > 0) {
        publish("Power Meter_2/data", payload);
        Serial.printf("\n===== PUBLICADO: Power Meter_2 (%d amostras) =====\n", n);
        Serial.println(payload);
        Serial.println();
    }
    free(payload);
