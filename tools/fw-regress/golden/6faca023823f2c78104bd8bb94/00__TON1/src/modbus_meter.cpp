...
static bool _read_dev_0_raw(uint16_t *buf) {
    ...
    // Bloco 0: reg 37, count 34, func 0x03 - V, I, P, Q, FP, S, Energia (37..70)
    {
        ...
        if (rc == _mb.ku8MBSuccess) {
            ...
            diag_modbus_err++;
            Serial.printf("[MB] Power_Meter_1 bloco 0 FAIL (reg=37 count=34 rc=0x%02X)\n", rc);
            return false;
        }
    }
...
static void _sample_dev_0() {
    ...
    if (!_read_dev_0_raw(buf)) {
        _ds0.fail_streak++;
        Serial.printf("[MB] Power_Meter_1: falha leitura (consecutivas: %d)\n", _ds0.fail_streak);
        if (_ds0.fail_streak >= MODBUS_FAIL_COOLDOWN_N) {
            ...
            Serial.printf("[MB] Power_Meter_1: cooldown %lus (nao responde) — loop liberado p/ LoRa/cmd\n",
    ...
    if (_ds0.fail_streak > 0) {
        Serial.printf("[MB] Power_Meter_1: OK (apos %d falhas consecutivas)\n", _ds0.fail_streak);
        _ds0.fail_streak = 0;
    }
    ...
    Serial.printf("[MB] Power_Meter_1 #%d: ", _ds0.samples);
...
static void _publish_dev_0(modbus_publish_fn publish) {
    if (!_ds0.valid || _ds0.samples == 0) {
        publish("Power_Meter_1/status", "{\"error\":\"no_samples\"}");
        return;
    }
    ...
    if (sz > 0) {
        ...
        Serial.printf("\n===== PUBLICADO: Power_Meter_1 (%d amostras) =====\n", n);
...
static bool _read_dev_1_raw(uint16_t *buf) {
    ...
    // Bloco 0: reg 37, count 34, func 0x03 - V, I, P, Q, FP, S, Energia (37..70)
    {
        ...
        if (rc == _mb.ku8MBSuccess) {
            ...
            diag_modbus_err++;
            Serial.printf("[MB] Power_Meter_2 bloco 0 FAIL (reg=37 count=34 rc=0x%02X)\n", rc);
            return false;
        }
    }
    // Espacamento entre blocos. 80ms cobre datalogger interno do Sungrow CX
...
static void _sample_dev_1() {
    ...
    if (!_read_dev_1_raw(buf)) {
        _ds1.fail_streak++;
        Serial.printf("[MB] Power_Meter_2: falha leitura (consecutivas: %d)\n", _ds1.fail_streak);
        if (_ds1.fail_streak >= MODBUS_FAIL_COOLDOWN_N) {
            ...
            Serial.printf("[MB] Power_Meter_2: cooldown %lus (nao responde) — loop liberado p/ LoRa/cmd\n",
                          (unsigned long)(MODBUS_COOLDOWN_MS/1000));
        }
        return;
    }
    ...
    if (_ds1.fail_streak > 0) {
        Serial.printf("[MB] Power_Meter_2: OK (apos %d falhas consecutivas)\n", _ds1.fail_streak);
        _ds1.fail_streak = 0;
    }
    ...
    float v_phf = (float)buf[27] / 1000.0;
    float v_consumo_phf = (float)buf[27] / 1000.0;
    if (!_ds1.has_first_delta) { _ds1.first_delta_[0] = v_consumo_phf; }
    float v_consumo_phr = (float)buf[29] / 1000.0;
    if (!_ds1.has_first_delta) { _ds1.first_delta_[1] = v_consumo_phr; }
    float v_consumo_qhf = (float)buf[31] / 1000.0;
    if (!_ds1.has_first_delta) { _ds1.first_delta_[2] = v_consumo_qhf; }
    float v_consumo_qhr = (float)buf[33] / 1000.0;
    if (!_ds1.has_first_delta) { _ds1.first_delta_[3] = v_consumo_qhr; }
    if (!_ds1.has_first_delta) _ds1.has_first_delta = true;
    _ds1.last_[0] = v_ia;
    _ds1.last_[1] = v_ib;
    _ds1.last_[2] = v_ic;
    _ds1.last_[3] = v_pt;
    ...
    _ds1.last_[8] = v_vc;
    _ds1.last_[9] = v_fp_a;
    _ds1.last_[10] = v_fp_b;
    _ds1.last_[11] = v_fp_c;
    _ds1.last_[12] = v_fp_t;
    _ds1.last_[13] = v_freq;
    _ds1.last_[14] = v_phf;
    _ds1.last_[15] = v_consumo_phf;
    _ds1.last_[16] = v_consumo_phr;
    _ds1.last_[17] = v_consumo_qhf;
    _ds1.last_[18] = v_consumo_qhr;
    _ds1.samples++;
    // Log de debug — mostra principais valores lidos
    Serial.printf("[MB] Power_Meter_2 #%d: ", _ds1.samples);
    Serial.printf("va=%.2f ", v_va);
    Serial.printf("vb=%.2f ", v_vb);
    Serial.printf("vc=%.2f ", v_vc);
    Serial.printf("ia=%.2f ", v_ia);
...
static void _publish_dev_1(modbus_publish_fn publish) {
    if (!_ds1.valid || _ds1.samples == 0) {
        publish("Power_Meter_2/status", "{\"error\":\"no_samples\"}");
        return;
    }
    ...
    float dv_consumo_phf = _ds1.last_[15] - _ds1.first_delta_[0];
    if (dv_consumo_phf < 0 || fabsf(dv_consumo_phf) < 1e-6f) dv_consumo_phf = 0;
    float dv_consumo_phr = _ds1.last_[16] - _ds1.first_delta_[1];
    if (dv_consumo_phr < 0 || fabsf(dv_consumo_phr) < 1e-6f) dv_consumo_phr = 0;
    float dv_consumo_qhf = _ds1.last_[17] - _ds1.first_delta_[2];
    if (dv_consumo_qhf < 0 || fabsf(dv_consumo_qhf) < 1e-6f) dv_consumo_qhf = 0;
    float dv_consumo_qhr = _ds1.last_[18] - _ds1.first_delta_[3];
    if (dv_consumo_qhr < 0 || fabsf(dv_consumo_qhr) < 1e-6f) dv_consumo_qhr = 0;
    d["phf"] = _ds1.last_[14];
    d["consumo_phf"] = dv_consumo_phf;
    d["consumo_phr"] = dv_consumo_phr;
    d["consumo_qhf"] = dv_consumo_qhf;
    d["consumo_qhr"] = dv_consumo_qhr;
    d["Va"] = (float)(_ds1.sum_[6] / n);
    d["Vb"] = (float)(_ds1.sum_[7] / n);
    d["Vc"] = (float)(_ds1.sum_[8] / n);
    d["Ia"] = (float)(_ds1.sum_[0] / n);
    d["Ib"] = (float)(_ds1.sum_[1] / n);
    d["Ic"] = (float)(_ds1.sum_[2] / n);
    d["FPa"] = (float)(_ds1.sum_[9] / n);
    d["FPb"] = (float)(_ds1.sum_[10] / n);
    d["FPc"] = (float)(_ds1.sum_[11] / n);
    d["Pt"] = (float)(_ds1.sum_[3] / n);
    d["Qt"] = (float)(_ds1.sum_[4] / n);
    d["St"] = (float)(_ds1.sum_[5] / n);
    d["Freq"] = (float)(_ds1.sum_[13] / n);
    d["FPt"] = (float)(_ds1.sum_[12] / n);
    d["timestamp"] = (long)time(nullptr);
    ...
    size_t sz = serializeJson(d, payload, 3072);
    if (sz > 0) {
        publish("Power_Meter_2/data", payload);
        Serial.printf("\n===== PUBLICADO: Power_Meter_2 (%d amostras) =====\n", n);
        Serial.println(payload);
        Serial.println();
    }
    free(payload);
    ...
    // Reset do ciclo (preserva 'first_delta' da ultima amostra para o proximo intervalo)
    _ds1.first_delta_[0] = _ds1.last_[15];
    _ds1.first_delta_[1] = _ds1.last_[16];
    _ds1.first_delta_[2] = _ds1.last_[17];
    _ds1.first_delta_[3] = _ds1.last_[18];
    for (int i = 0; i < 14; i++) _ds1.sum_[i] = 0;
    _ds1.samples = 0;
}
...
static bool _read_dev_2_raw(uint16_t *buf) {
    ...
    // Bloco 0: reg 37, count 34, func 0x03 - V, I, P, Q, FP, S, Energia (37..70)
    {
        ...
        if (rc == _mb.ku8MBSuccess) {
            ...
            diag_modbus_err++;
            Serial.printf("[MB] Power_Meter_3 bloco 0 FAIL (reg=37 count=34 rc=0x%02X)\n", rc);
            return false;
        }
    }
    // Espacamento entre blocos. 80ms cobre datalogger interno do Sungrow CX
...
static void _sample_dev_2() {
    ...
    if (!_read_dev_2_raw(buf)) {
        _ds2.fail_streak++;
        Serial.printf("[MB] Power_Meter_3: falha leitura (consecutivas: %d)\n", _ds2.fail_streak);
        if (_ds2.fail_streak >= MODBUS_FAIL_COOLDOWN_N) {
            ...
            Serial.printf("[MB] Power_Meter_3: cooldown %lus (nao responde) — loop liberado p/ LoRa/cmd\n",
                          (unsigned long)(MODBUS_COOLDOWN_MS/1000));
        }
        return;
    }
    ...
    if (_ds2.fail_streak > 0) {
        Serial.printf("[MB] Power_Meter_3: OK (apos %d falhas consecutivas)\n", _ds2.fail_streak);
        _ds2.fail_streak = 0;
    }
    ...
    float v_phf = (float)buf[27] / 1000.0;
    float v_consumo_phf = (float)buf[27] / 1000.0;
    if (!_ds2.has_first_delta) { _ds2.first_delta_[0] = v_consumo_phf; }
    float v_consumo_phr = (float)buf[29] / 1000.0;
    if (!_ds2.has_first_delta) { _ds2.first_delta_[1] = v_consumo_phr; }
    float v_consumo_qhf = (float)buf[31] / 1000.0;
    if (!_ds2.has_first_delta) { _ds2.first_delta_[2] = v_consumo_qhf; }
    float v_consumo_qhr = (float)buf[33] / 1000.0;
    if (!_ds2.has_first_delta) { _ds2.first_delta_[3] = v_consumo_qhr; }
    if (!_ds2.has_first_delta) _ds2.has_first_delta = true;
    _ds2.last_[0] = v_ia;
    _ds2.last_[1] = v_ib;
    _ds2.last_[2] = v_ic;
    _ds2.last_[3] = v_pt;
    ...
    _ds2.last_[8] = v_vc;
    _ds2.last_[9] = v_fp_a;
    _ds2.last_[10] = v_fp_b;
    _ds2.last_[11] = v_fp_c;
    _ds2.last_[12] = v_fp_t;
    _ds2.last_[13] = v_freq;
    _ds2.last_[14] = v_phf;
    _ds2.last_[15] = v_consumo_phf;
    _ds2.last_[16] = v_consumo_phr;
    _ds2.last_[17] = v_consumo_qhf;
    _ds2.last_[18] = v_consumo_qhr;
    _ds2.samples++;
    // Log de debug — mostra principais valores lidos
    Serial.printf("[MB] Power_Meter_3 #%d: ", _ds2.samples);
    Serial.printf("va=%.2f ", v_va);
    Serial.printf("vb=%.2f ", v_vb);
    Serial.printf("vc=%.2f ", v_vc);
    Serial.printf("ia=%.2f ", v_ia);
...
static void _publish_dev_2(modbus_publish_fn publish) {
    if (!_ds2.valid || _ds2.samples == 0) {
        publish("Power_Meter_3/status", "{\"error\":\"no_samples\"}");
        return;
    }
    ...
    float dv_consumo_phf = _ds2.last_[15] - _ds2.first_delta_[0];
    if (dv_consumo_phf < 0 || fabsf(dv_consumo_phf) < 1e-6f) dv_consumo_phf = 0;
    float dv_consumo_phr = _ds2.last_[16] - _ds2.first_delta_[1];
    if (dv_consumo_phr < 0 || fabsf(dv_consumo_phr) < 1e-6f) dv_consumo_phr = 0;
    float dv_consumo_qhf = _ds2.last_[17] - _ds2.first_delta_[2];
    if (dv_consumo_qhf < 0 || fabsf(dv_consumo_qhf) < 1e-6f) dv_consumo_qhf = 0;
    float dv_consumo_qhr = _ds2.last_[18] - _ds2.first_delta_[3];
    if (dv_consumo_qhr < 0 || fabsf(dv_consumo_qhr) < 1e-6f) dv_consumo_qhr = 0;
    d["phf"] = _ds2.last_[14];
    d["consumo_phf"] = dv_consumo_phf;
    d["consumo_phr"] = dv_consumo_phr;
    d["consumo_qhf"] = dv_consumo_qhf;
    d["consumo_qhr"] = dv_consumo_qhr;
    d["Va"] = (float)(_ds2.sum_[6] / n);
    d["Vb"] = (float)(_ds2.sum_[7] / n);
    d["Vc"] = (float)(_ds2.sum_[8] / n);
    d["Ia"] = (float)(_ds2.sum_[0] / n);
    d["Ib"] = (float)(_ds2.sum_[1] / n);
    d["Ic"] = (float)(_ds2.sum_[2] / n);
    d["FPa"] = (float)(_ds2.sum_[9] / n);
    d["FPb"] = (float)(_ds2.sum_[10] / n);
    d["FPc"] = (float)(_ds2.sum_[11] / n);
    d["Pt"] = (float)(_ds2.sum_[3] / n);
    d["Qt"] = (float)(_ds2.sum_[4] / n);
    d["St"] = (float)(_ds2.sum_[5] / n);
    d["Freq"] = (float)(_ds2.sum_[13] / n);
    d["FPt"] = (float)(_ds2.sum_[12] / n);
    d["timestamp"] = (long)time(nullptr);
    ...
    size_t sz = serializeJson(d, payload, 3072);
    if (sz > 0) {
        publish("Power_Meter_3/data", payload);
        Serial.printf("\n===== PUBLICADO: Power_Meter_3 (%d amostras) =====\n", n);
        Serial.println(payload);
        Serial.println();
    }
    free(payload);
    ...
    // Reset do ciclo (preserva 'first_delta' da ultima amostra para o proximo intervalo)
    _ds2.first_delta_[0] = _ds2.last_[15];
    _ds2.first_delta_[1] = _ds2.last_[16];
    _ds2.first_delta_[2] = _ds2.last_[17];
    _ds2.first_delta_[3] = _ds2.last_[18];
    for (int i = 0; i < 14; i++) _ds2.sum_[i] = 0;
    _ds2.samples = 0;
}
