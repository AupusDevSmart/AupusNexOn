...
static _Dev3State _ds3 = {};

static bool _read_dev_3_raw(uint16_t *buf) {
    _select(4);
    // Bloco 0: reg 4999, count 50, func 0x04 - Regs 5000-5049: info, yields, MPPT 1-3, V/I AC, potencias, status
    {
        uint8_t rc = _mb.readInputRegisters(4999, 50);
        ...
        if (rc == _mb.ku8MBSuccess) {
            ...
            diag_modbus_err++;
            Serial.printf("[MB] Inversor_4 bloco 0 FAIL (reg=4999 count=50 rc=0x%02X)\n", rc);
            return false;
        }
    }
    // Espacamento entre blocos. 80ms cobre datalogger interno do Sungrow CX
    ...
    // Bloco 1: reg 5049, count 40, func 0x04 - Regs 5050-5089: regulation, insulation
    {
        ...
        if (rc == _mb.ku8MBSuccess) {
            ...
            diag_modbus_err++;
            Serial.printf("[MB] Inversor_4 bloco 1 FAIL (reg=5049 count=40 rc=0x%02X)\n", rc);
            return false;
        }
    }
    // Espacamento entre blocos. 80ms cobre datalogger interno do Sungrow CX
    ...
    // Bloco 2: reg 5089, count 30, func 0x04 - Regs 5090-5119: tempo diario, MPPT 4-6
    {
        ...
        if (rc == _mb.ku8MBSuccess) {
            ...
            diag_modbus_err++;
            Serial.printf("[MB] Inversor_4 bloco 2 FAIL (reg=5089 count=30 rc=0x%02X)\n", rc);
            return false;
        }
    }
    // Espacamento entre blocos. 80ms cobre datalogger interno do Sungrow CX
    ...
    // Bloco 3: reg 5119, count 35, func 0x04 - Regs 5120-5154: MPPT 7-9, bus voltage, PID
    {
        ...
        if (rc == _mb.ku8MBSuccess) {
            ...
            diag_modbus_err++;
            Serial.printf("[MB] Inversor_4 bloco 3 FAIL (reg=5119 count=35 rc=0x%02X)\n", rc);
            return false;
        }
    }
    // Espacamento entre blocos. 80ms cobre datalogger interno do Sungrow CX
    ...
    // Bloco 4: reg 7012, count 18, func 0x04 - Regs 7013-7030: strings 1-18
    {
        ...
        if (rc == _mb.ku8MBSuccess) {
            ...
            diag_modbus_err++;
            Serial.printf("[MB] Inversor_4 bloco 4 FAIL (reg=7012 count=18 rc=0x%02X)\n", rc);
            return false;
        }
    }
    // Espacamento entre blocos. 80ms cobre datalogger interno do Sungrow CX
...
// Le + acumula. Chamar a cada READ_INTERVAL_MS.
static void _sample_dev_3() {
    // Back-off: device que falhou MODBUS_FAIL_COOLDOWN_N vezes fica em cooldown.
    // Pula a leitura (que bloquearia o loop no timeout) ate o cooldown expirar.
    if (_ds3.cooldown_until && (long)(millis() - _ds3.cooldown_until) < 0) return;

    uint16_t buf[173];
    if (!_read_dev_3_raw(buf)) {
        _ds3.fail_streak++;
        Serial.printf("[MB] Inversor_4: falha leitura (consecutivas: %d)\n", _ds3.fail_streak);
        if (_ds3.fail_streak >= MODBUS_FAIL_COOLDOWN_N) {
            _ds3.cooldown_until = millis() + MODBUS_COOLDOWN_MS;
            if (_ds3.cooldown_until == 0) _ds3.cooldown_until = 1;  // 0 = sentinela "sem cooldown"
            Serial.printf("[MB] Inversor_4: cooldown %lus (nao responde) — loop liberado p/ LoRa/cmd\n",
                          (unsigned long)(MODBUS_COOLDOWN_MS/1000));
        }
        return;
    }
    diag_last_successful_read_ms = millis();
    _ds3.cooldown_until = 0;
    if (_ds3.fail_streak > 0) {
        Serial.printf("[MB] Inversor_4: OK (apos %d falhas consecutivas)\n", _ds3.fail_streak);
        _ds3.fail_streak = 0;
    }
    ...
    _ds3.sum_[5] += v_vca;
    float v_mppt1_voltage = (float)buf[11] / 10.0;
    _ds3.sum_[6] += v_mppt1_voltage;
    float v_mppt2_voltage = (float)buf[13] / 10.0;
    _ds3.sum_[7] += v_mppt2_voltage;
    float v_mppt3_voltage = (float)buf[15] / 10.0;
    _ds3.sum_[8] += v_mppt3_voltage;
    float v_mppt4_voltage = (float)buf[115] / 10.0;
    _ds3.sum_[9] += v_mppt4_voltage;
    float v_mppt5_voltage = (float)buf[117] / 10.0;
    _ds3.sum_[10] += v_mppt5_voltage;
    float v_mppt6_voltage = (float)buf[119] / 10.0;
    _ds3.sum_[11] += v_mppt6_voltage;
    float v_mppt7_voltage = (float)buf[121] / 10.0;
    _ds3.sum_[12] += v_mppt7_voltage;
    float v_mppt8_voltage = (float)buf[123] / 10.0;
    _ds3.sum_[13] += v_mppt8_voltage;
    float v_mppt9_voltage = (float)buf[130] / 10.0;
    _ds3.sum_[14] += v_mppt9_voltage;
    float v_dc_total_power = (float)(((uint32_t)buf[17+1] << 16) | buf[17]);
    _ds3.sum_[15] += v_dc_total_power;
    float v_string1_current = (float)buf[155] / 100.0;
    _ds3.sum_[16] += v_string1_current;
    float v_string2_current = (float)buf[156] / 100.0;
    _ds3.sum_[17] += v_string2_current;
    float v_string3_current = (float)buf[157] / 100.0;
    _ds3.sum_[18] += v_string3_current;
    float v_string4_current = (float)buf[158] / 100.0;
    _ds3.sum_[19] += v_string4_current;
    float v_string5_current = (float)buf[159] / 100.0;
    _ds3.sum_[20] += v_string5_current;
    float v_string6_current = (float)buf[160] / 100.0;
    _ds3.sum_[21] += v_string6_current;
    float v_string7_current = (float)buf[161] / 100.0;
    _ds3.sum_[22] += v_string7_current;
    float v_string8_current = (float)buf[162] / 100.0;
    _ds3.sum_[23] += v_string8_current;
    float v_string9_current = (float)buf[163] / 100.0;
    _ds3.sum_[24] += v_string9_current;
    float v_string10_current = (float)buf[164] / 100.0;
    _ds3.sum_[25] += v_string10_current;
    float v_string11_current = (float)buf[165] / 100.0;
    _ds3.sum_[26] += v_string11_current;
    float v_string12_current = (float)buf[166] / 100.0;
    _ds3.sum_[27] += v_string12_current;
    float v_string13_current = (float)buf[167] / 100.0;
    _ds3.sum_[28] += v_string13_current;
    float v_string14_current = (float)buf[168] / 100.0;
    _ds3.sum_[29] += v_string14_current;
    float v_string15_current = (float)buf[169] / 100.0;
    _ds3.sum_[30] += v_string15_current;
    float v_string16_current = (float)buf[170] / 100.0;
    _ds3.sum_[31] += v_string16_current;
    float v_string17_current = (float)buf[171] / 100.0;
    _ds3.sum_[32] += v_string17_current;
    float v_string18_current = (float)buf[172] / 100.0;
    _ds3.sum_[33] += v_string18_current;
    float v_fp = (float)(int16_t)buf[35] / 1000.0;
    float v_freq = (float)buf[36] / 10.0;
    float v_work_state = (float)buf[38];
    float v_bus_voltage = (float)buf[147] / 10.0;
    ...
    _ds3.last_[4] = v_vbc;
    _ds3.last_[5] = v_vca;
    _ds3.last_[6] = v_mppt1_voltage;
    _ds3.last_[7] = v_mppt2_voltage;
    _ds3.last_[8] = v_mppt3_voltage;
    _ds3.last_[9] = v_mppt4_voltage;
    _ds3.last_[10] = v_mppt5_voltage;
    _ds3.last_[11] = v_mppt6_voltage;
    _ds3.last_[12] = v_mppt7_voltage;
    _ds3.last_[13] = v_mppt8_voltage;
    _ds3.last_[14] = v_mppt9_voltage;
    _ds3.last_[15] = v_dc_total_power;
    _ds3.last_[16] = v_string1_current;
    _ds3.last_[17] = v_string2_current;
    _ds3.last_[18] = v_string3_current;
    _ds3.last_[19] = v_string4_current;
    _ds3.last_[20] = v_string5_current;
    _ds3.last_[21] = v_string6_current;
    _ds3.last_[22] = v_string7_current;
    _ds3.last_[23] = v_string8_current;
    _ds3.last_[24] = v_string9_current;
    _ds3.last_[25] = v_string10_current;
    _ds3.last_[26] = v_string11_current;
    _ds3.last_[27] = v_string12_current;
    _ds3.last_[28] = v_string13_current;
    _ds3.last_[29] = v_string14_current;
    _ds3.last_[30] = v_string15_current;
    _ds3.last_[31] = v_string16_current;
    _ds3.last_[32] = v_string17_current;
    _ds3.last_[33] = v_string18_current;
    _ds3.last_[34] = v_fp;
    _ds3.last_[35] = v_freq;
    _ds3.last_[36] = v_work_state;
    _ds3.last_[37] = v_bus_voltage;
    _ds3.last_[38] = v_daily_yield;
    _ds3.last_[39] = v_device_type;
    _ds3.last_[40] = v_output_type;
    _ds3.last_[41] = v_total_yield;
    _ds3.last_[42] = v_temp_interna;
    _ds3.last_[43] = v_nominal_power;
    _ds3.last_[44] = v_apparent_total;
    _ds3.last_[45] = v_pid_alarm_code;
    _ds3.last_[46] = v_pid_work_state;
    _ds3.last_[47] = v_potencia_ativa;
    _ds3.last_[48] = v_potencia_reativa;
    _ds3.last_[49] = v_potencia_aparente;
    _ds3.last_[50] = v_daily_running_time;
    _ds3.last_[51] = v_potencia_aparente2;
    _ds3.last_[52] = v_total_running_time;
    _ds3.last_[53] = v_insulation_resistance;
    _ds3.last_[54] = v_nominal_reactive_power;
    _ds3.samples++;
    // Log de debug — mostra principais valores lidos
    Serial.printf("[MB] Inversor_4 #%d: ", _ds3.samples);
    Serial.printf("vab=%.2f ", v_vab);
    Serial.printf("vbc=%.2f ", v_vbc);
    Serial.printf("vca=%.2f ", v_vca);
    Serial.printf("ia=%.2f ", v_ia);
...
// Publica JSON (medias + last + delta) e reseta acumuladores. Chamar a cada PUBLISH_INTERVAL_MS.
static void _publish_dev_3(modbus_publish_fn publish) {
    if (!_ds3.valid || _ds3.samples == 0) {
        publish("Inversor_4/status", "{\"error\":\"no_samples\"}");
        return;
    }
    ...
    JsonObject g_info = d.createNestedObject("info");
    { char _h[8]; snprintf(_h, sizeof(_h), "%x", (uint16_t)(_ds3.last_[39])); g_info["device_type"] = String(_h); }
    g_info["output_type"] = _ds3.last_[40];
    g_info["nominal_power"] = _ds3.last_[43];
    JsonObject g_energy = d.createNestedObject("energy");
    g_energy["daily_yield"] = _ds3.last_[38];
    g_energy["total_yield"] = _ds3.last_[41];
    g_energy["Potencia Aparente1"] = _ds3.last_[49];
    g_energy["daily_running_time"] = _ds3.last_[50];
    g_energy["Potencia Aparente2"] = _ds3.last_[51];
    g_energy["total_running_time"] = _ds3.last_[52];
    JsonObject g_temperature = d.createNestedObject("temperature");
    g_temperature["internal"] = _ds3.last_[42];
    JsonObject g_dc = d.createNestedObject("dc");
    g_dc["mppt1_voltage"] = (float)(_ds3.sum_[6] / n);
    g_dc["mppt2_voltage"] = (float)(_ds3.sum_[7] / n);
    g_dc["mppt3_voltage"] = (float)(_ds3.sum_[8] / n);
    g_dc["mppt4_voltage"] = (float)(_ds3.sum_[9] / n);
    g_dc["mppt5_voltage"] = (float)(_ds3.sum_[10] / n);
    g_dc["mppt6_voltage"] = (float)(_ds3.sum_[11] / n);
    g_dc["mppt7_voltage"] = (float)(_ds3.sum_[12] / n);
    g_dc["mppt8_voltage"] = (float)(_ds3.sum_[13] / n);
    g_dc["mppt9_voltage"] = (float)(_ds3.sum_[14] / n);
    g_dc["total_power"] = (float)(_ds3.sum_[15] / n);
    g_dc["string1_current"] = (float)(_ds3.sum_[16] / n);
    g_dc["string2_current"] = (float)(_ds3.sum_[17] / n);
    g_dc["string3_current"] = (float)(_ds3.sum_[18] / n);
    g_dc["string4_current"] = (float)(_ds3.sum_[19] / n);
    g_dc["string5_current"] = (float)(_ds3.sum_[20] / n);
    g_dc["string6_current"] = (float)(_ds3.sum_[21] / n);
    g_dc["string7_current"] = (float)(_ds3.sum_[22] / n);
    g_dc["string8_current"] = (float)(_ds3.sum_[23] / n);
    g_dc["string9_current"] = (float)(_ds3.sum_[24] / n);
    g_dc["string10_current"] = (float)(_ds3.sum_[25] / n);
    g_dc["string11_current"] = (float)(_ds3.sum_[26] / n);
    g_dc["string12_current"] = (float)(_ds3.sum_[27] / n);
    g_dc["string13_current"] = (float)(_ds3.sum_[28] / n);
    g_dc["string14_current"] = (float)(_ds3.sum_[29] / n);
    g_dc["string15_current"] = (float)(_ds3.sum_[30] / n);
    g_dc["string16_current"] = (float)(_ds3.sum_[31] / n);
    g_dc["string17_current"] = (float)(_ds3.sum_[32] / n);
    g_dc["string18_current"] = (float)(_ds3.sum_[33] / n);
    JsonObject g_voltage = d.createNestedObject("voltage");
    g_voltage["phase_a-b"] = (float)(_ds3.sum_[3] / n);
    g_voltage["phase_b-c"] = (float)(_ds3.sum_[4] / n);
    g_voltage["phase_c-a"] = (float)(_ds3.sum_[5] / n);
    JsonObject g_current = d.createNestedObject("current");
    g_current["phase_a"] = (float)(_ds3.sum_[0] / n);
    g_current["phase_b"] = (float)(_ds3.sum_[1] / n);
    g_current["phase_c"] = (float)(_ds3.sum_[2] / n);
    JsonObject g_power = d.createNestedObject("power");
    g_power["power_factor"] = _ds3.last_[34];
    g_power["frequency"] = _ds3.last_[35];
    g_power["apparent_total"] = _ds3.last_[44];
    g_power["active_total"] = _ds3.last_[47];
    g_power["reactive_total"] = _ds3.last_[48];
    JsonObject g_status = d.createNestedObject("status");
    g_status["work_state"] = _ds3.last_[36];
    g_status["work_state_text"] = _work_state_text((uint16_t)_ds3.last_[36]);
    JsonObject g_protection = d.createNestedObject("protection");
    g_protection["bus_voltage"] = _ds3.last_[37];
    g_protection["insulation_resistance"] = _ds3.last_[53];
    JsonObject g_regulation = d.createNestedObject("regulation");
    g_regulation["nominal_reactive_power"] = _ds3.last_[54];
    JsonObject g_pid = d.createNestedObject("pid");
    g_pid["alarm_code"] = _ds3.last_[45];
    g_pid["work_state"] = _ds3.last_[46];
    ...
    size_t sz = serializeJson(d, payload, 3072);
    if (sz > 0) {
        publish("Inversor_4/data", payload);
        Serial.printf("\n===== PUBLICADO: Inversor_4 (%d amostras) =====\n", n);
        Serial.println(payload);
        Serial.println();
    }
    free(payload);
    ...
    // Reset do ciclo (preserva 'first_delta' da ultima amostra para o proximo intervalo)
    for (int i = 0; i < 34; i++) _ds3.sum_[i] = 0;
    _ds3.samples = 0;
}

static int _rr_idx = 0;
static const int _dev_count = 4;
