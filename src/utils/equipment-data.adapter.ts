/**
 * ADAPTER PATTERN - Normalização de Dados de Equipamentos (Frontend)
 *
...
export function detectDataFormat(data: RawEquipmentData): 'power-meter' | 'inverter' | 'legacy' | 'unknown' {
  ...
  // Inversor: tem power.active, voltage.phase_a, etc.
  if (data.power?.active !== undefined || data.voltage?.phase_a !== undefined) {
    return 'inverter';
  }
...
export class InverterAdapter {
  static normalize(data: RawEquipmentData): NormalizedElectricalData {
    ...
    return {
      voltage: {
        L1: voltage.phase_a || voltage['phase_a-b'] || 0,
        L2: voltage.phase_b || voltage['phase_b-c'] || 0,
        L3: voltage.phase_c || voltage['phase_c-a'] || 0,
        average: ((voltage.phase_a || 0) + (voltage.phase_b || 0) + (voltage.phase_c || 0)) / 3,
      },
      current: {
