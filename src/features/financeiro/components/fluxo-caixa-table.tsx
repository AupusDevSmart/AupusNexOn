...
export function FluxoCaixaTable({
  ...
  const meses: (keyof FluxoCaixaDetailed)[] = [
    "jan",
    "fev",
    "mar",
    "abr",
  ...
  const getFilteredMonths = () => {
    const startIndex = meses.indexOf(
      dateFilters.startMonth as keyof FluxoCaixaDetailed
    );
    const endIndex = meses.indexOf(
      dateFilters.endMonth as keyof FluxoCaixaDetailed
    );
    return {
  ...
  const { months: filteredMonths, labels: filteredLabels } =
    getFilteredMonths();
  ...
  const getRowClass = (
    ...
    const hoverClass =
      "hover:bg-slate-100/70 dark:hover:bg-gray-600/50 transition-colors duration-200";
            ...
            <table className="w-full table-fixed border-collapse">
              <thead>
                <tr className="bg-slate-100/80 dark:bg-gray-700/50 border-b-2 border-slate-200 dark:border-gray-600">
                  <th
                    className="text-left py-3 sm:py-5 px-4 sm:px-6 font-semibold text-slate-700 dark:text-gray-300"
                  ...
                  {filteredLabels.map((mes, index) => (
                    <th
                      key={mes}
                      className="text-center py-3 sm:py-5 px-2 font-semibold text-slate-700 dark:text-gray-300"
                      style={{ width: `${monthWidth}px` }}
                    >
                      <div className="flex flex-col items-center space-y-1 sm:space-y-2">
                        <span className="text-sm sm:text-base">{mes}</span>
                        <div
                          className="text-xs text-slate-500 dark:text-gray-400 font-normal bg-slate-200/50 dark:bg-gray-600/50 px-1 py-1 rounded-md truncate max-w-full"
                          title={formatCurrency(
                            calculateTotal(filteredMonths[index])
                          )}
                        >
              ...
              <tbody>
                {orderedData.map((row, index) => (
                  <tr key={index} className={getRowClass(row.tipo, index)}>
                    <td
                      className="py-3 sm:py-4 px-4 sm:px-6 font-medium"
                      style={{ width: `${moduleWidth}px` }}
                    >
                      <div className="flex items-center space-x-2 sm:space-x-3 min-w-0">
                        <div
                          className={`${getCellTextClass(
                            row.tipo
                          )} flex-shrink-0`}
                        >
                        ...
                        <span
                          className={`${getCellTextClass(
                            row.tipo
                          )} text-sm sm:text-base truncate flex-1 min-w-0`}
                          title={row.modulo}
                        >
                          {row.modulo}
                        </span>
                    ...
                    {filteredMonths.map((mes) => {
                      ...
                      return (
                        <td
                          key={mes}
                          className="text-center py-3 sm:py-4 px-2"
                          style={{ width: `${monthWidth}px` }}
                        >
                          <div className="flex flex-col items-center justify-center space-y-1 min-w-0">
                            ...
                            <span
                              className={`${getCellTextClass(
                                row.tipo,
                                value
                              )} text-xs sm:text-sm truncate max-w-full`}
