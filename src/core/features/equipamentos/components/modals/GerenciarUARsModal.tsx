...
import { useEquipamentos } from '@/core/context/hooks';
      ...
      {!loadingData && !errorLocal && uarsLista.length > 0 && (
        <div className="grid grid-cols-1 gap-2">
          {uarsLista.map((uar) => {
            ...
            return (
              <div key={uar.id} className="rounded-lg border bg-card p-4 transition-shadow hover:shadow-sm">
                <div className="flex items-start justify-between gap-4">
                  ...
                  <div className="flex gap-0.5 text-muted-foreground transition-colors">
                    <button
                    ...
                    <button
                      ...
                      className="btn-minimal-ghost h-8 w-8 p-0 hover:text-destructive"
                      title="Remover"
                    >
