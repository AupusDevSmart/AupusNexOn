...
export function MapaUsinas({ usinas, onUsinaClick, usinaSelecionada }: MapaUsinasProps) {
  ...
  useEffect(() => {
    if (mapInstanceRef.current && typeof window !== 'undefined' && (window as any).L) {
      ...
      usinas.forEach((usina) => {
        ...
        // Criar marcador
        const marker = L.marker([usina.lat, usina.lng], { icon })
          .addTo(map)
          .bindPopup(`
            <div style="min-width: 200px;">
              <h3 style="margin: 0 0 8px 0; font-weight: bold;">${usina.nome}</h3>
              <p style="margin: 4px 0;"><strong>Potência:</strong> ${usina.potencia} kW</p>
              <p style="margin: 4px 0;"><strong>Status:</strong> ${
                usina.status === 'operacao' ? 'Em Operação' :
                usina.status === 'alerta' ? 'Em Alerta' : 'Com Falha'
              }</p>
              <button 
      ...
      (window as any).abrirDetalhesUsina = (id: number) => {
        ...
        window.dispatchEvent(new CustomEvent('abrirModalUsina', { detail: { id } }));
      };
    }
      ...
      <div className="relative">
        ...
        <div className="mt-4 flex flex-wrap items-center justify-center gap-6">
          <div className="flex items-center gap-2">
            ...
            ></div>
            <span className="text-sm text-muted-foreground">Ativos em operação</span>
          </div>
          <div className="flex items-center gap-2">
            <div 
              className="w-3 h-3 rounded-full"
            ...
            ></div>
            <span className="text-sm text-muted-foreground">Ativos em alerta</span>
          </div>
          <div className="flex items-center gap-2">
            <div 
              className="w-3 h-3 rounded-full"
            ...
            ></div>
            <span className="text-sm text-muted-foreground">Com falha/offline</span>
          </div>
        </div>
      </div>
