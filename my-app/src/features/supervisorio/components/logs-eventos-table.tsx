...
import { Eye, Settings, CheckSquare, FileText, MoreHorizontal } from 'lucide-react';
...
export function LogsEventosTable({
  ...
  const getTipoEventoColor = (tipo: string) => {
    switch (tipo) {
      case 'TRIP': return 'bg-red-500 text-white';
      case 'URGENCIA': return 'bg-orange-500 text-white';
      case 'ALARME': return 'bg-yellow-500 text-white';
      case 'MANUTENCAO': return 'bg-blue-500 text-white';
      case 'INFORMATIVO': return 'bg-gray-500 text-white';
            ...
            <TableHeader>
              <TableRow>
                <TableHead className="w-12">
                  <Checkbox
                    checked={selectedItems.length === eventos.length && eventos.length > 0}
            ...
            <TableBody>
              {eventos.map((evento) => (
                <TableRow key={evento.id}>
                  ...
                  <TableCell>
                    <Badge variant="outline" className={getSeveridadeColor(evento.severidade)}>
                      {evento.severidade}
                    </Badge>
                  </TableCell>
                  ...
                  <TableCell>
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                        <DropdownMenuItem onClick={() => onVerDetalhes(evento)}>
                          <Eye className="mr-2 h-4 w-4" />
                          Ver Detalhes
                        </DropdownMenuItem>
                        <DropdownMenuItem onClick={() => onAssociarOS(evento)}>
                          <FileText className="mr-2 h-4 w-4" />
                          Associar OS
                        </DropdownMenuItem>
                        {!evento.reconhecido && (
                          <DropdownMenuItem onClick={() => onMarcarReconhecido(evento)}>
                            <CheckSquare className="mr-2 h-4 w-4" />
